...
const Chamadas: React.FC<ChamadasProps> = ({ onNotify }) => {
  ...
  useEffect(() => {
    if (selectedTurnoId) {
      loadChamadaMilitar(selectedTurnoId);
    } else {
      setChamadaMilitar([]);
    }
  }, [selectedTurnoId]);
  ...
  const loadInitialData = async () => {
    try {
      setLoading(true);
      const [turnosData, militaresData] = await Promise.all([
  ...
  const loadChamadaMilitar = async (idTurno: string) => {
    try {
      const data = await apiService.getChamadaMilitar(idTurno);
      setChamadaMilitar(data);
    } catch (error) {
      onNotify?.("Erro ao carregar chamada militar.", "error");
    }
  };
  ...
  const addSelectedMilitares = async () => {
    if (!selectedTurnoId || pendingMilitares.length === 0) return;

    try {
      const currentIds = chamadaMilitar.map(m => m.matricula.toLowerCase());
      const newEntries = pendingMilitares
        .filter(m => !currentIds.includes(m.toLowerCase()))
        .map(matricula => ({
      ...
      if (newEntries.length > 0) {
        await Promise.all(newEntries.map(entry => apiService.createChamadaMilitar(entry)));
        loadChamadaMilitar(selectedTurnoId);
      ...
      onNotify?.("Erro ao escalar militares.", "error");
    }
  };

  const updateMilitarChamada = async (id: string, updates: Partial<ChamadaMilitar>) => {
    try {
              ...
              {isMenuOpen && (
                <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl rounded-xl z-[999] p-4 animate-in fade-in zoom-in duration-200">
                  ...
                  <div className="space-y-1 max-h-64 overflow-y-auto mb-4 border-b dark:border-slate-800 pb-2 custom-scrollbar">
                    {militares
                      .filter(m => !chamadaMilitar.some(cm => cm.matricula === m.matricula))
                      .filter(m => m.nome_completo.toLowerCase().includes(militarSearchTerm.toLowerCase()) || m.nome_guerra.toLowerCase().includes(militarSearchTerm.toLowerCase()))
                      .map(m => (
                        <button
                          key={m.matricula}
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <span className="font-bold block text-slate-900 dark:text-white">{m.nome_posto_grad} {m.nome_guerra}</span>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {chamadaMilitar.map(cm => {
                  ...
                  return m ? (
                    <tr key={cm.id_chamada_militar} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                      <td className="p-4">
                        <div className="font-bold text-slate-900 dark:text-white">{m.nome_posto_grad} {m.nome_guerra}</div>
