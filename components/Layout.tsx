...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab }) => {
  ...
  const navItems = [
    ...
    { id: 'diario', label: 'Diário de Operações', icon: <FileText size={20} />, roles: [UserRole.ADMIN, UserRole.OPERADOR] },
    ...
    { id: 'usuarios', label: 'Config. Usuários', icon: <Settings size={20} />, roles: [UserRole.ADMIN] },
  ];

  const filteredNavItems = navItems.filter(item => user && item.roles.includes(user.role));

  return (
    <div className={`min-h-screen flex flex-col md:flex-row transition-colors duration-500 ${isDarkMode ? 'bg-[#020617] text-slate-100' : 'bg-[#F8FAFC] text-slate-900'}`}>
      {/* Mobile Header */}
      <header className="md:hidden bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 backdrop-blur-md p-4 flex justify-between items-center sticky top-0 z-50">
      ...
      `}>
        ...
        <nav className="mt-4 px-4 space-y-2 overflow-y-auto h-[calc(100vh-320px)] scrollbar-hide">
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-x-hidden p-6 md:p-12 relative">
